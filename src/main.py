...
class Widget(QMainWindow):
    def __init__(self, parent=None):
        ...
        self.usersettings_filename = os.path.expanduser("~/Library/Application Support/TAMUSPE/settings.yaml")
        self.saved_data_filename = os.path.expanduser("~/Library/Application Support/TAMUSPE/last_saved_data.parquet")

        self.setting_filename = os.path.join(os.path.dirname(__file__), 'etc/settings.yaml')
        self.setting_ui_filename = os.path.join(os.path.dirname(__file__), 'etc/settings.ui')
        self.feedback_ui_filename = os.path.join(os.path.dirname(__file__), 'etc/feedback.ui')
        self.infowidget_ui_filename = os.path.join(os.path.dirname(__file__), 'etc/info_widget.ui')
        print(self.setting_ui_filename)
    ...
    def app_start(self):
        if not os.path.exists(self.usersettings_filename):
            os.makedirs(os.path.dirname(self.usersettings_filename), exist_ok=True)
            shutil.copy(self.setting_filename, self.usersettings_filename)
            settings = changeyaml.pull(self.usersettings_filename)
            user_id = str(uuid.uuid4())
            settings['user_id'] = user_id
            changeyaml.push(self.usersettings_filename, settings)
        ...
        if is_dark_mode():
            ...
            icon = QIcon()
            icon.addFile(":/Images/images/icons8-refresh-24.png", QSize(), QIcon.Normal, QIcon.Off)
            self.ui.refresh.setIcon(icon)
            self.ui.refresh.setIconSize(QSize(16, 16))

            icon1 = QIcon()
            icon1.addFile(":/Images/images/blk-settings-50.png", QSize(), QIcon.Normal, QIcon.Off)
    ...
    def update_data(self):
        ...
        try:
            self.df = data()
            self.df.to_parquet(self.saved_data_filename)
            self.update_table()
            self.ui.offline_label.setText('')
            self.online = True

            if not changeyaml.pull(self.usersettings_filename)['saved_username']:
                import get_key
                key = get_key.get()
                if key == 'error':
                    pass
                    ...
                    settings = changeyaml.pull(self.usersettings_filename)
                    response = store_userid_version.send(key, f"{settings['user_id']}_mac", settings['version'])
                    if response == 'error':
        ...
        # offline mode
            ...
            try:
                self.df = pd.read_parquet(self.saved_data_filename)
                self.update_table()
            except:
                pass
    ...
    def update_table(self):
            ...
            for col in range(0, len(cols)):  # Start from the third column
                if col == 0:
                    ...
                    if text == 'None':
                        ...
                        font.setBold(True)
                        item = QTableWidgetItem('Double Click')
        ...
        for col in range(self.table.horizontalHeader().count()-1):
            ...
            self.table.horizontalHeader().resizeSection(col, current_size + 10)

        print('table updated')

    def item_double_clicked(self, item):
        if (self.table.item(item.row(), 4).text() == "Double Click" or
                self.table.item(item.row(), 1).text() == "Double Click"):
    ...
    def start_notification_timer(self):
        if changeyaml.pull(self.usersettings_filename)['notifications']:
    ...
    def send_notification(self, test=None):
        ...
        notification_hours = changeyaml.pull(self.usersettings_filename)['notification_hours']
        notification_days = changeyaml.pull(self.usersettings_filename)['notification_days']
# glass
        if test:
            pync.notify(title=title, subtitle='Registration Required',
                        message=message, open='https://www.tamuspe.org/calendar', sound='Pop')

        if location != "--":
            print('time_difference', time_difference)

            if changeyaml.pull(self.usersettings_filename)['days']:
                ...
                if time_difference == timedelta(days=notification_days):
                    pync.notify(title=title, subtitle=subtitle,
                                message=message, open='https://www.tamuspe.org/calendar', sound='Glass')

                    print(f'The start time is less than {notification_days} days away from the current time.')

            if changeyaml.pull(self.usersettings_filename)['hours']:
    ...
    def open_info_widget(self, row):
        ...
        loader = QUiLoader()
        info_dialog = loader.load(self.infowidget_ui_filename, self)
    ...
    def open_settings(self):
        tim = time.time()
        loader = QUiLoader()
        settings_dialog = loader.load(self.setting_ui_filename, self)
        ...
        def load_current_settings():
            cur_set = changeyaml.pull(self.usersettings_filename)
        ...
        def accept(cur_set):
            ...
            changeyaml.push(self.usersettings_filename, cur_set)
            print(cur_set)
    ...
    def feedback_dialog(self):
        loader = QUiLoader()
        feedback_dialog = loader.load(self.feedback_ui_filename, self)
        ...
        def send():
            print('feedback: send')
            user_id = changeyaml.pull(self.usersettings_filename)['user_id']
            filename = os.path.join(os.path.dirname(__file__),
                                    f'{user_id}_{datetime.now().strftime("%Y-%m-%d_%I:%M:%S%p")}.txt')
    ...
    def keep_in_background(self):
        ...
        tray_icon.setToolTip("Your App Tooltip")  # Set your app tooltip here
        ...
        def open_event(row):
            ...
            if col4 == "Double Click" or col1 == "Double Click":
    ...
    def closeEvent(self, event):
        if (self.background_running and not self.background_quit and
                changeyaml.pull(self.usersettings_filename)['open_in_background']):
