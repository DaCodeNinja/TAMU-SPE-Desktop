...
class TerminalNotifier(object):
    ...
    def __init__(self, wait=False):
        ...
        """
        self._wait = wait
        proc = subprocess.Popen(["which", "tamuspe-notifier"], stdout=subprocess.PIPE)
        ...
        elif os.path.exists("/usr/local/bin/tamuspe-notifier"):
            self.bin_path = os.path.join("/usr/local/bin/", "tamuspe-notifier")
        else:
